...
@router.post("/trips/{trip_id}/start", response_model=type(get_trip_details("")))
async def start_trip(trip_id: str):
    ...
    """
    # Validate booking/trip exists
    if trip_id not in BOOKINGS_DB:
    ...
    if trip_id not in STARTED_TRIPS:
        ...
        # Determine destination_id from booking
        booking = BOOKINGS_DB[trip_id]
        destination_id = booking.homestay.destination_id if booking and hasattr(booking, "homestay") else None
