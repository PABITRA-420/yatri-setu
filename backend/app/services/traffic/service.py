...
class TrafficService:
    ...
    def _initialize_provider(self) -> BaseTrafficProvider:
        ...
        if mode in ("real", "tomtom", "google"):
            real_p = RealTrafficProvider()
            if real_p.is_available():
                return real_p
            logger.info("Real traffic API key missing; falling back to DemoTrafficProvider")
    ...
    def get_traffic(self, destination_id: str, force_refresh: bool = False) -> DestinationTrafficSummary:
        ...
        try:
            ...
            logger.warning(f"Traffic provider error for {dest_clean}: {ex}. Checking stale cache fallback.")
            if cached_entry:
            ...
            fallback = demo.fetch_traffic(dest_clean)
