...
export default function DestinationDetailsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      {isOvercrowded && (
        <div className="bg-rose-50 dark:bg-rose-950/30 border border-rose-300 dark:border-rose-900/60 rounded-[24px] p-6 sm:p-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 shadow-xs">
          <div className="flex items-start gap-4">
            <div className="p-3 rounded-[20px] bg-rose-700 text-white shrink-0 mt-0.5">
          ...
          <Link
            ...
            className="px-5 py-3 rounded-[20px] bg-rose-700 hover:bg-rose-800 text-white font-bold text-xs shadow-md active:scale-97 transition-all flex items-center gap-2 whitespace-nowrap self-stretch sm:self-auto justify-center"
          >
      ...
      <div className="relative rounded-[2.5rem] overflow-hidden shadow-[0_20px_55px_rgba(23,23,20,0.10)] min-h-[420px] sm:min-h-[500px] flex items-end bg-stone-950">
        <img
        ...
        <div className="relative z-10 p-8 sm:p-12 w-full flex flex-col md:flex-row md:items-end justify-between gap-6 text-white">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <Link
              ...
              className="px-5 py-3.5 rounded-[20px] bg-amber-700 hover:bg-amber-800 text-white text-xs font-bold shadow-lg flex items-center gap-2 transition-all active:scale-97"
            >
            ...
            <Link
              href={`/destinations/${destination.id}/alternatives`}
              className="glass-pill px-5 py-3.5 rounded-[20px] text-white text-xs font-bold flex items-center gap-2 transition-all hover:bg-[#FCFAF6]/30"
            >
              <Sparkles className="w-4 h-4 text-amber-300" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          {/* Overview */}
          <div className="bg-[#FCFAF6] dark:bg-[#121824] rounded-[24px] p-7 sm:p-8 border border-black/[.08]/80 dark:border-white/10 shadow-xs space-y-4">
            <h2 className="text-xl font-extrabold text-[#171714] dark:text-white tracking-tight">
              Destination Overview
            </h2>
            <p className="text-sm text-[#706E68] dark:text-stone-300 leading-relaxed">
            ...
            <div className="pt-3 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {destination.highlights.map((highlight, idx) => (
                  <div key={idx} className="flex items-start gap-2.5 text-xs text-[#171714]/75 dark:text-stone-300 bg-[#F5F2EA] dark:bg-stone-900/40 p-2.5 rounded-xl border border-black/[.08]/60 dark:border-white/5">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {destination.attractions.map((attraction) => (
                <div
                  key={attraction.id}
                  className="editorial-card bg-[#FCFAF6] dark:bg-[#121824] rounded-[24px] p-5 border border-black/[.08]/80 dark:border-white/10 shadow-xs flex flex-col sm:flex-row gap-5"
                >
                  <img
                    src={attraction.image_url}
                    alt={attraction.name}
                    className="w-full sm:w-44 h-32 rounded-[20px] object-cover"
                    loading="lazy"
                  />
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between gap-2">
                        <h4 className="font-extrabold text-base text-[#171714] dark:text-white tracking-tight">
                          {attraction.name}
                        </h4>
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full ${
                          attraction.crowd_density === 'High' ? 'bg-rose-50 text-rose-700 border border-rose-200 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-900/60' : 'bg-emerald-50 text-[#183A32] border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-900/60'
                        }`}>
                      ...
                      <p className="text-xs text-[#706E68] dark:text-stone-400 mt-1.5 leading-relaxed">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#183A32] dark:text-emerald-400">
            ...
            {homestaysLoading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-pulse">
                {[1, 2].map((i) => (
                  <div key={i} className="h-80 bg-stone-200 dark:bg-stone-800 rounded-[24px]" />
                ))}
              </div>
            ) : homestays.length === 0 ? (
              <EmptyState />
        ...
        <div className="lg:col-span-4 space-y-6">
          {/* Live Crowd Card */}
          {crowd && (
            <div className="bg-[#FCFAF6] dark:bg-[#121824] rounded-[24px] p-6 sm:p-7 border border-black/[.08]/80 dark:border-white/10 shadow-xs text-center space-y-5">
              ...
              <div className="pt-4 border-t border-stone-100 dark:border-white/5 text-left space-y-2.5 text-xs text-[#706E68]">
                <div className="flex justify-between">
                  <span>Peak Visiting:</span>
                  <span className="font-bold text-[#171714] dark:text-stone-200">{crowd.peak_visiting_hours}</span>
                </div>
                <div className="flex justify-between">
                  <span>Hotel Occupancy:</span>
                  <span className="font-bold text-[#171714] dark:text-stone-200 font-mono">{crowd.hotel_occupancy_rate}</span>
                </div>
                <div className="flex justify-between">
                  <span>Road Traffic:</span>
                  <span className="font-bold text-[#171714] dark:text-stone-200">{crowd.live_traffic_status}</span>
                </div>
              </div>

              <Link
                href={`/destinations/${destination.id}/crowd`}
                className="w-full py-3 rounded-[20px] bg-[#F5F2EA] dark:bg-stone-800 hover:bg-stone-200/80 dark:hover:bg-stone-700 text-[#171714] dark:text-white text-xs font-bold block transition-colors"
              >
          ...
          {/* Quick Actions Card */}
          <div className="bg-stone-950 text-white rounded-[24px] p-7 shadow-[0_12px_40px_rgba(23,23,20,0.06)] border border-stone-800 space-y-4">
            ...
            <div className="space-y-3 pt-2">
              <Link
                ...
                className="w-full py-3 rounded-[20px] bg-amber-400 hover:bg-amber-300 text-[#171714] text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-xs active:scale-97"
              >
              ...
              <Link
                href={`/homestays?destination_id=${destination.id}`}
                className="w-full py-3 rounded-[20px] bg-[#FCFAF6]/10 hover:bg-[#FCFAF6]/15 text-white text-xs font-bold flex items-center justify-center gap-2 transition-colors border border-white/10"
              >
