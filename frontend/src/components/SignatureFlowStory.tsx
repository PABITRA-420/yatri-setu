  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2 sm:pt-4 pb-20 sm:pb-28 space-y-12">
        ...
        {/* ═══════════════════════════════════════════════════════════════════════ */}
        {/* 3-DAY DISTRIBUTED JOURNEY ITINERARY                                   */}
        {/* ═══════════════════════════════════════════════════════════════════════ */}
        <div className={cn(
          'relative rounded-[1.75rem] overflow-hidden transition-all duration-1000 delay-700',
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        )}>
          {/* Card border glow */}
          <div className="absolute inset-0 rounded-[1.75rem] bg-gradient-to-br from-amber-500/15 via-white/[0.03] to-emerald-500/10 p-[1px]">
            <div className="w-full h-full rounded-[1.75rem] bg-stone-950/90 backdrop-blur-2xl" />
          </div>

          <div className="relative z-10 p-8 sm:p-10 space-y-8">

            {/* Header Row */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-7 border-b border-white/[0.06]">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2.5">
                  <Compass className="w-5 h-5 text-amber-400" />
                  <span className="text-[11px] font-mono uppercase tracking-[0.2em] text-amber-400 font-bold">
                    YOUR JOURNEY • DISTRIBUTED ROUTE
                  </span>
                </div>
                <h4 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
                  3 DAYS: NATURE • CULTURE • LOCAL STAYS
                </h4>
              </div>
              <div className="px-4 py-2 rounded-xl bg-amber-400/10 border border-amber-400/20 text-amber-300 font-mono font-bold text-xs tracking-wider">
                3 DAYS • 2 NIGHTS
              </div>
            </div>

            {/* 3 Day Cards — vertical timeline on mobile, horizontal on desktop */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {[
                {
                  day: '01',
                  phase: 'ARRIVAL & SETTLE',
                  title: 'Teesta Valley & Orchid Ridge',
                  desc: 'Bypass Hill Cart Road queues; check into certified village cottage overlooking terraced orchids.',
                  accent: 'from-amber-500/10 to-amber-600/5',
                  border: 'border-amber-500/10',
                },
                {
                  day: '02',
                  phase: 'MONASTERY & RIDGES',
                  title: 'Deolo Ridge & Zang Dhok',
                  desc: 'Unobstructed 360° Kanchenjunga sunrise with zero dawn traffic; afternoon monastic art trails.',
                ...
                },
                {
                  ...
                  title: 'Lava Pine Canopies & Host Table',
                  desc: '90% tariff stays with local host family; earn +30 platform Green Credits for verified decongestion.',
                  accent: 'from-teal-500/10 to-sky-600/5',
                ...
                <div
                  key={d.day}
                  className={cn(
                    'relative p-5 rounded-2xl border bg-gradient-to-b overflow-hidden group/day transition-all duration-300 hover:border-white/[0.12] hover:shadow-lg',
                    d.border,
                    d.accent
                  )}
                >
                  {/* Day number watermark */}
                  <span className="absolute -bottom-3 -right-1 text-[5rem] font-black font-mono text-white/[0.03] leading-none select-none pointer-events-none">
                    {d.day}
                  </span>
                  <div className="relative space-y-2.5">
                    <div className="flex items-center justify-between text-[11px] font-mono">
                      <span className="font-bold text-amber-400 tracking-wider">DAY {d.day}</span>
                      <span className="text-stone-500 tracking-wider">{d.phase}</span>
                    </div>
            ...
            {/* Bottom CTA */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-5 pt-6 border-t border-white/[0.06]">
              <p className="text-xs text-stone-500 max-w-md text-center sm:text-left leading-relaxed">
                Includes dynamic weather alerts, verified homestay host contact, and automated offline SOS credentials.
              </p>

              <Link
                href="/destinations/darjeeling/alternatives"
                className="w-full sm:w-auto px-8 h-13 rounded-2xl bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 hover:from-amber-300 hover:via-amber-400 hover:to-amber-500 text-stone-950 font-extrabold text-sm tracking-wide shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 transition-all duration-300 group/cta"
              >
                <span>Discover my alternative</span>
